...
export class SalesInteractionService {
  ...
  addSales( drugName: Array<any> =[], totalPrice: number, tax: number, paidAmount: number, balance: number) {
    ...
    this.http.post<{message: string, salesId: string}>('https://apipms.demo-kota.com/api/sales',sales)
  ...
  getSales() {
    this.http.get<{message: string, sales: any}>('https://apipms.demo-kota.com/api/sales')
    .pipe(map(salesData => {
  ...
  getSalesChartInfo2():Observable<any>{
    ...
    return this.http.get<{ message: string,sales:any}>('https://apipms.demo-kota.com/api/sales/getSalesChartInfo');
   ...
   getSalesChartInfo(){
    console.log("service")
    this.http.get<{message: string, sales: any}>('https://apipms.demo-kota.com/api/sales/getSalesChartInfo')
    .pipe(map(salesData => {
     return salesData.sales.map(sales=>{
       return{
        // drugName: sales.drugName,
  ...
  //   this.http
  //            .put('https://apipms.demo-kota.com/api/supplier/' + id , supplier)
  //            .subscribe(response => {
